...
public class PollinationBatchReader implements BatchReader {
	...
	public void readConfig(String filename) throws IOException {
		...
		output = new Printer(temp_filename1,"nP\tnP1\tnP2\tnPa\tnPb\tnVa\tnVb\tnOv1\tnOv2\tnFl1\tnFl2\tnPo1\tnPo2\tlossa\tlossb\tconv_tol\tbn\tSteps\tASteps\tMP1Aa\tMP2Aa\tMP1Ab\tMP2Ab\tMP1Fa\tMP2Fa\tMP1Fb\tMP2Fb", false);
		output.printHeader();
		line = buff.readLine();
		line = buff.readLine();
		while ( line != null ) {
			if (!line.startsWith(new String("#")))
			{
				...
				commands.add(new RunParamObj(temp_filename1, temp_filename, temp_num_boots, temp_steps,temp_plants, temp_num_plants_1, temp_num_plants_2, temp_num_pollinator_a, temp_num_pollinator_b, temp_num_visits_a, temp_num_visits_b, temp_num_ovules_1, temp_num_ovules_2,temp_num_flowers_1,temp_num_flowers_2, temp_num_pollen_grain_1, temp_num_pollen_grain_2,temp_pollen_loss_rate_a, temp_pollen_loss_rate_b, temp_convergence_tolerance ));
			}
			line = buff.readLine();
		}
	}
	...
	public void run() {
		...
		for ( int i = 0; i < commands.size(); i++ ) {
			...
			try {
				outfile = new Printer(temp.get_filename(),"bootn\ttime\tpid\tptype\tnum_pollen\tnum_st_pollen\tnum_pollen_right\tnum_pollen_wrong\tnum_pollen_lost\tnum_pollen_on_pollinator\tfit_a\tfit_b\tattract_a\tattract_b", false);
				outfile.printHeader();
				...
				{	
					test = new Model (temp.get_masterfile(),
							...
							temp.get_conv_tol();
					System.out.println(temp.get_filename() +"\t" + ii +"\t"+ temp.get_num_steps()+"\t"+ temp.get_num_plants()+"\t"+temp.get_num_plants_1()+"\t"+ temp.get_num_plants_2()+"\t"+ temp.get_num_pollinator_a() + "\t" + temp.get_num_pollinator_b() + "\t" + temp.get_num_visits_a()+"\t"+temp.get_num_visits_b()+"\t"+temp.get_num_ovules_1()+"\t"+temp.get_num_ovules_2()+"\t"+temp.get_num_flowers_1()+"\t"+temp.get_num_flowers_2()+"\t"+temp.get_num_pollen_grain_1()+"\t"+ temp.get_num_pollen_grain_2()+"\t"+temp.get_pollen_loss_rate_a()+"\t"+temp.get_pollen_loss_rate_b() + "\t" + temp.get_conv_tol());
					test.run(ii, temp.get_num_steps(), temp.get_num_plants_1(), temp.get_num_plants_2(), temp.get_num_pollinator_a(), temp.get_num_pollinator_b(), temp.get_num_visits_a(), temp.get_num_visits_b(),temp.get_pollen_loss_rate_a(), temp.get_pollen_loss_rate_b(), temp.get_conv_tol());	
				}
				} 
			catch (FileNotFoundException e) 
			{
